...
async def test_send_request_to_member(ac: AsyncClient, users_tokens):
    headers = {
        "Authorization": f"Bearer {users_tokens['test1@test.com']}",
    }
    payload = {
...
async def test_send_request_exist(ac: AsyncClient, users_tokens):
    ...
    assert response.json().get('detail') == "Request is already sent"
...
async def test_requests_company_one_not_owner(ac: AsyncClient, users_tokens):
    ...
    assert response.status_code == 403
    assert response.json().get('detail') == "You are not owner in this company"
...
async def test_cancel_requests_not_found(ac: AsyncClient, users_tokens):
    ...
    assert response.status_code == 404
    assert response.json().get('detail') == "Action does not exist"
############################################################################################################
...
async def test_cancel_requests_not_your(ac: AsyncClient, users_tokens):
    ...
    response = await ac.delete("/request/5", headers=headers)
    assert response.status_code == 403
...
async def test_decline_request_not_owner(ac: AsyncClient, users_tokens):
    ...
    response = await ac.get('/request/6/decline', headers=headers)
    assert response.status_code == 403
...
async def test_accept_requests(ac: AsyncClient, users_tokens):
    ...
    response = await ac.get("/request/7/accept", headers=headers)
    assert response.status_code == 200
