...
auth_routers = APIRouter(tags=["auth0"])


@auth_routers.get('/auth/me', response_model=ResultUserToken, status_code=status.HTTP_200_OK)
...
@auth_routers.post("/auth/login", response_model=TokenResponse, status_code=status.HTTP_200_OK)
